...
const RadioCard: React.FC<RadioCardProps> = React.memo(({ answer, isSelected, onSelect }) => {
        ...
        >
            <Flex align='center'>
                <Box
                    ...
                    flexShrink={0}
                >
                    {isSelected && <Box w='12px' h='12px' borderRadius='full' bg='white' />}
                </Box>
                <Text fontSize='md' fontWeight='medium' color='gray.800' flex='1'>
                    {answer.title}
                </Text>
            </Flex>
...
export const StandardTestView: React.FC<StandardTestViewProps> = ({ test, onComplete }) => {
    ...
    const result: TestResult | undefined = test.results?.find(
        (r: TestResult) => totalScore >= r.min && totalScore <= r.max,
    );
        ...
        return (
            <VStack spacing={6} align='stretch'>
                ...
                <Stack direction={{ base: 'column', sm: 'row' }} spacing={4}>
                    <Button colorScheme='blue' size='lg' flex='1' py={2} onClick={handleReset}>
                        Пройти заново
                    </Button>
                    <Button colorScheme='green' size='lg' flex='1' py={2} onClick={onComplete}>
                    ...
                    {test.questions?.map((questionItem: TestQuestion, index: number) => {
                        ...
                        return (
                            <Box
                                ...
                                h='36px'
                                borderRadius='md'
                                bg={isAnswered ? 'green.500' : isCurrent ? 'blue.500' : 'gray.200'}
                                color='white'
                                display='flex'
                                alignItems='center'
                                justifyContent='center'
